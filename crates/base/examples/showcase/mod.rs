...
fn showcase_editor_style() -> InputEditorStyle {
    InputEditorStyle::new()
        .with_foreground(example_rgb(0x171717).into())
        .with_muted_foreground(example_rgb(0x737373).into())
        .with_selection(gpui::hsla(0.6, 0.8, 0.7, 0.45))
        .with_caret(CaretStyle::new().with_color(example_rgb(0x171717).into()))
}
...
impl BaseShowcase {
    ...
    fn refresh_editor_styles(&self, cx: &mut Context<Self>) {
        let style = showcase_editor_style;
        self.input
        ...
        self.editor.update(cx, |state, _| {
            state
                .set_editor_style(style().with_highlight_styles(Arc::new(ShowcaseHighlightStyles)));
        });
    }
