...
mod tests {
    ...
    fn from_theme_maps_base_semantic_tokens() {
        let mut tokens = crate::SemanticThemeTokens::default();
        tokens.colors.foreground = gpui::rgb(0x112233).into();
        tokens.colors.muted_foreground = gpui::rgb(0x445566).into();
        tokens.colors.primary = gpui::rgb(0x3366ff).into();
        tokens.colors.accent = gpui::rgb(0xddeeff).into();
        tokens.colors.border = gpui::rgb(0x778899).into();
        tokens.colors.selection = gpui::rgb(0x55a0fc).into();
        let theme = crate::Theme::new().with_tokens(tokens);
