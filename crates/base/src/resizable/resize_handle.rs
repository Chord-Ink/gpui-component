...
pub(crate) fn handle_color(theme: &crate::Theme, active: bool) -> gpui::Hsla {
    if active {
        ...
        theme
            .resizable()
            .handle()
            .unwrap_or(theme.tokens().colors.border)
    }
}
...
mod tests {
    ...
    fn an_unprojected_handle_resolves_from_the_theme_tokens(cx: &mut TestAppContext) {
        cx.update(|cx| {
            ...
            Theme::update(cx, |theme| {
                ...
                tokens.colors.border = border;
                tokens.colors.ring = ring;
                theme
                    .with_tokens(tokens)
                    .with_resizable(ResizableTheme::new())
            });
    ...
    fn a_projected_handle_still_wins(cx: &mut TestAppContext) {
        cx.update(|cx| {
            ...
            Theme::update(cx, |theme| {
                let mut tokens = theme.tokens().clone();
                tokens.colors.border = hsla(0., 0., 0.5, 1.0);
                theme.with_tokens(tokens).with_resizable(
                    ResizableTheme::new()
                        .with_handle(projected)
                        .with_active_handle(active),
                )
            });
