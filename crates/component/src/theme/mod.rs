...
impl Theme {
    ...
    fn base_theme(&self) -> gpui_base::Theme {
        gpui_base::Theme::new()
            .with_appearance(if self.mode.is_dark() {
    ...
    pub fn color_tokens(&self) -> ColorTokens {
        ColorTokens {
            ...
            input: self.input,
            ring: self.ring,
            // Resolved, not raw: Base paints text selection from this token, so
            // it has to carry the platform color the styled layer would use.
...
mod base_theme_projection_tests {
    ...
    fn default_component_palettes_match_base_light_and_dark_tokens(cx: &mut gpui::TestAppContext) {
        fn assert_close(left: ColorTokens, right: ColorTokens) {
            ...
            color!(input);
            color!(ring);
            // `selection` is left out: this theme names no `selection.background`,
            // so the token follows the platform rather than the Base palette.
        }
